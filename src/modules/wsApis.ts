...
const connectApi = nodeConnection.getNodeConnection().then((api) => {
    return api;
  });
  const connectDb = dataBaseConnection.getDbConnection().then((db) => {
    ...
    return db;
       ...
       exports.LastBlock= async () => {
        ...
        exports.getTransactionsCount = async () => {
            ...
            const result = await connectDb.query(`SELECT COUNT(*) AS count FROM transactions`);
            return  result?.rows;
        }
